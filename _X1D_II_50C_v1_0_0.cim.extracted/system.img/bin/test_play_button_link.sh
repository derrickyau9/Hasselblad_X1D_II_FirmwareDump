...
eagle_play_button_link_test()
{
    local event="/dev/input/event4"
    local play_button_keyword="KEY_F5"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$play_button_keyword" /blackbox/system/event.log
                #if it is not play_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass play button link test"
                    #open event2
    ...
    if [ -f /blackbox/system/event.log ]; then
        grep "$play_button_keyword" /blackbox/system/event.log
        if [ $? != 0 ]; then
            echo "no play button operation"
            return 1
        else
            echo "pass paly button link test"
            return 0
        fi
    else
