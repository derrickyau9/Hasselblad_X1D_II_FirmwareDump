...
eagle_ae_button_link_test()
{
    local event="/dev/input/event5"
    local ae_button_keyword="KEY_L"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$ae_button_keyword" /blackbox/system/event.log
                #if it is not ae_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass AE-L button link test"
                    #open event2
