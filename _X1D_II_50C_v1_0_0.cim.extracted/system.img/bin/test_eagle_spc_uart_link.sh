...
eagle_spc_uart_link_test()
{
    local keyword="firmware_version :"
    upgrade_fw -c -n pwrctrl > /blackbox/system/test_tmp.log 2>&1
    if [ $? != 0 ]; then
        echo "eagle to spc test fail"
        return 1
    else
        if [ -f /blackbox/system/test_tmp.log ]; then
            ...
            if [ $? != 0 ]; then
                ...
                echo "eagle to spc test pass"
                return 0
            fi
        else
            echo "eagle to spc test fail,no test_tmp.log"
            return 3
        fi
    fi
}
