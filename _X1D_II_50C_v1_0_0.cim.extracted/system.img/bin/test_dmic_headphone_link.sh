...
display_audio()
{
    ...
    if [ $? != 0 ]; then
        echo "external headphone display audio fail"
        return 15
    fi
    echo "external headphone display audio success"
    return 0
}
...
#display std audio
echo "start to display std audio"
display_std_audio
if [ $? != 0 ]; then
    exit $?
fi
...
echo "start to capture audio"
capture_audio 16 48000
if [ $? != 0 ]; then
    exit $?
fi
#display capture audio
echo "start to display audio"
display_audio
exit $?
