...
eagle_delete_button_link_test()
{
    local event="/dev/input/event4"
    local delete_button_keyword="KEY_F2"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$delete_button_keyword" /blackbox/system/event.log
                #if it is not delete_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass delete button link test"
                    #open event2
    ...
    if [ -f /blackbox/system/event.log ]; then
        grep "$delete_button_keyword" /blackbox/system/event.log
        if [ $? != 0 ]; then
            echo "no delete button operation"
            return 1
        else
            echo "pass delete button link test"
            return 0
