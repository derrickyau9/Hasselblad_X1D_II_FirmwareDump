...
eagle_video_setting_button_link_test()
{
    local event="/dev/input/event4"
    local video_setting_button_keyword="KEY_F3"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$video_setting_button_keyword" /blackbox/system/event.log
                #if it is not video_setting_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass video setting button link test"
                    #open event2
    ...
    if [ -f /blackbox/system/event.log ]; then
        grep "$video_setting_button_keyword" /blackbox/system/event.log
        if [ $? != 0 ]; then
            echo "no video setting button operation"
            return 1
        else
            echo "pass video setting button link test"
            return 0
