...
eagle_menu_button_link_test()
{
    local event="/dev/input/event4"
    local menu_button_keyword="KEY_F1"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$menu_button_keyword" /blackbox/system/event.log
                #if it is not menu_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass menu button link test"
                    #open event2
