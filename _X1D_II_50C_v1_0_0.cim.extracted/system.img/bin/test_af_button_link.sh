...
eagle_af_button_link_test()
{
    local event="/dev/input/event5"
    local af_button_keyword="KEY_D"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$af_button_keyword" /blackbox/system/event.log
                #if it is not af_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass AF-D button link test"
                    #open event2
