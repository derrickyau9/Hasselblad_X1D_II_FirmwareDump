...
eagle_camera_setting_button_link_test()
{
    local event="/dev/input/event4"
    local camera_setting_button_keyword="KEY_F4"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$camera_setting_button_keyword" /blackbox/system/event.log
                #if it is not camera_setting_button try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass camera setting button link test"
                    #open event2
    ...
    if [ -f /blackbox/system/event.log ]; then
        grep "$camera_setting_button_keyword" /blackbox/system/event.log
        if [ $? != 0 ]; then
            echo "no camera setting button operation"
            return 1
        else
            echo "pass camera setting button link test"
            return 0
