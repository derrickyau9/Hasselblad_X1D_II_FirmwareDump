...
eagle_headphone_link_test()
{
    local event="/dev/input/event7"
    local headphone_keyword="SW_HEADPHONE_INSERT"
    ...
    do
        ...
        if [ -z $ID ];then
            if [ -f /blackbox/system/event.log ]; then
                grep "$headphone_keyword" /blackbox/system/event.log
                #if it is not headphone try again
                if [ $? != 0 ]; then
                    getevent -l -c 1 > /blackbox/system/event.log &
                    continue
                else
                    echo "pass headphone link test"
                    #open event2
